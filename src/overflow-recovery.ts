  ...
  ctx.on(
    ...
    async ({ agent, failure, signal }, next): Promise<RequestErrorAction> => {
      ...
      try {
        ...
        ctx.logger.warn(
          `overflow recovery: built-in handler threw: ${error instanceof Error ? error.message : String(error)}`,
        );
        action = undefined;
      }
      ...
      try {
        ...
        ctx.logger.info(
          `overflow recovery: compacted ${result.shadowedSeqs.length} surface nodes; retrying request`,
        );
        return { kind: "retry" };
      } catch (error) {
        ctx.logger.warn(
          `overflow recovery failed: ${error instanceof Error ? error.message : String(error)}`,
        );
        return undefined;
      }
    },
