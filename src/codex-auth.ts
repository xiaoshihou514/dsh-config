...
async function readAuth(path: string): Promise<CodexAuthFile> {
  ...
  try {
    ...
    throw new Error(`未找到 Codex 登录信息（${path}）；请先运行 codex login`, {
      cause: error,
    });
  }
  try {
    ...
    throw new Error(`Codex 登录文件不是有效的 JSON（${path}）`, {
      cause: error,
    });
  }
}
...
function credentials(auth: CodexAuthFile): OAuthCredential {
  if (auth.auth_mode !== "chatgpt") {
    throw new Error(
      "当前 Codex 不是通过 ChatGPT 订阅登录；请运行 codex login 并选择 ChatGPT 登录",
    );
  }
