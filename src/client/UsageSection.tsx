...
const price: Record<Model, Price> = {
  "deepseek-v4-flash": { hit: { peak: 0.1, off: 0.05 }, miss: { peak: 3, off: 1.5 }, output: { peak: 9, off: 4.5 } },
  "deepseek-v4-pro": { hit: { peak: 0.3, off: 0.15 }, miss: { peak: 9, off: 4.5 }, output: { peak: 27, off: 13.5 } }
};
...
function costOf(record: RecordItem): number {
  ...
  return (record.cacheReadTokens * unit.hit[tier]
    + (record.inputTokens + record.cacheWriteTokens) * unit.miss[tier]
    + record.outputTokens * unit.output[tier]) / 1_000_000;
}
...
function total(records: readonly RecordItem[]): Totals {
  ...
  }, { calls: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0, peakCost: 0, offCost: 0 });
}

function tokensOf(records: readonly RecordItem[]): number {
  return records.reduce((sum, record) => sum + record.inputTokens + record.outputTokens + record.cacheReadTokens + record.cacheWriteTokens, 0);
}
...
function fullLabel(start: number, view: View): string {
  ...
  if (view === "month") return new Intl.DateTimeFormat("zh-CN", { year: "numeric", month: "long" }).format(date);
  if (view === "week") return `${new Intl.DateTimeFormat("zh-CN", { year: "numeric", month: "long", day: "numeric" }).format(date)} 当周`;
...
function isNextDay(a: string, b: string): boolean {
  return new Date(`${b}T00:00:00`).getTime() - new Date(`${a}T00:00:00`).getTime() === DAY;
}

/** 连续使用天数：当前连续（截至最近有记录的一天）与历史最长连续。 */
function streaks(dayKeys: readonly string[]): { current: number; longest: number } {
...
function monthLabelFor(weekStart: number): string | undefined {
  ...
  const firstOfMonth = new Date(monday.getFullYear(), monday.getMonth(), 1).getTime();
...
function Metric({ icon, value, label: text }: { icon: ReactNode; value: string; label: string }) {
  return <div style={{ flex: "1 1 150px", minWidth: 0, border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 10, background: "var(--dsw-alias-bg-layer-2)", padding: "12px 14px" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--dsw-alias-label-tertiary)", fontSize: 12 }}>{icon}<span>{text}</span></div>
    <div style={{ marginTop: 6, fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em", whiteSpace: "nowrap" }}>{value}</div>
  </div>;
}
...
export function UsageSection() {
  ...
  const models = useMemo(() => ["全部模型", ...Array.from(new Set(records.map((record) => record.model))).sort()], [records]);
  ...
  const displayed = useMemo(() => view === "day"
    ...
    : view === "week"
      ...
      : monthBuckets.flatMap((bucket) => bucket.records), [view, dayWeeks, weekBuckets, monthBuckets]);
  ...
  const metrics = useMemo(() => {
    ...
    for (const record of filtered) {
      ...
      dayTotals.set(key, (dayTotals.get(key) ?? 0) + record.inputTokens + record.outputTokens + record.cacheReadTokens + record.cacheWriteTokens);
    }
  ...
  const weekMonthLabels = useMemo(() => {
    ...
    for (let column = 0; column < 4; column += 1) {
      ...
      for (const week of weeks) {
        ...
        if (candidate !== undefined) { text = candidate; break; }
      }
      labels.push({ left: (column / 4) * 100, text: text ?? `${new Date(first.start).getMonth() + 1}月` });
    }
    return labels;
  }, [weekBuckets]);
  ...
  return <section>
    <header style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
      <div>
        <h2 style={{ margin: 0, fontSize: 18, letterSpacing: "-0.02em", display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <label style={{ fontSize: 12, color: "var(--dsw-alias-label-tertiary)" }}>项目　<select value={project} onChange={(event) => { setProject(event.target.value); setSelected(null); }} style={{ color: "inherit", background: "transparent", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 7, padding: "5px 7px" }}>{projects.map((item) => <option key={item}>{item}</option>)}</select></label>
        <label style={{ fontSize: 12, color: "var(--dsw-alias-label-tertiary)" }}>模型　<select value={model} onChange={(event) => { setModel(event.target.value); setSelected(null); }} style={{ color: "inherit", background: "transparent", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 7, padding: "5px 7px" }}>{models.map((item) => <option key={item}>{item}</option>)}</select></label>
      </div>
    </header>
    ...
    <div style={{ marginTop: 18, border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 12, background: "var(--dsw-alias-bg-layer-3)", padding: "16px 16px 14px" }}>
      ...
      {view === "day" && <div style={{ marginTop: 16 }}>
        <div style={{ display: "flex", gap: DAY_GAP }}>
          <div style={{ display: "flex", flexDirection: "column", gap: DAY_GAP }}>
            {["一", "二", "三", "四", "五", "六", "日"].map((weekday, index) => <span key={index} style={{ height: DAY_CELL, width: 12, fontSize: 10, lineHeight: `${DAY_CELL}px`, textAlign: "center", color: "var(--dsw-alias-label-tertiary)" }}>{weekday}</span>)}
          </div>
        ...
        <div style={{ position: "relative", height: 15, marginTop: 3, marginLeft: 15 }}>
          {dayMonthLabels.map(({ left, text }) => <span key={text + left} style={{ position: "absolute", left: `${left}%`, fontSize: 10, color: "var(--dsw-alias-label-tertiary)", whiteSpace: "nowrap" }}>{text}</span>)}
        </div>
      </div>}

      {view === "week" && <div style={{ marginTop: 16 }}>
        <div style={{ display: "flex", gap: WEEK_GAP }}>
          {[0, 1, 2, 3].map((column) => <div key={column} style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: WEEK_GAP }}>
        ...
        <div style={{ position: "relative", height: 15, marginTop: 3 }}>
          {weekMonthLabels.map(({ left, text }) => <span key={text + left} style={{ position: "absolute", left: `${left}%`, fontSize: 10, color: "var(--dsw-alias-label-tertiary)", whiteSpace: "nowrap" }}>{text}</span>)}
        </div>
      </div>}

      {view === "month" && <div style={{ marginTop: 16, display: "flex", flexWrap: "wrap", gap: MONTH_GAP, maxWidth: 6 * MONTH_CELL + 5 * MONTH_GAP }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, color: "var(--dsw-alias-label-tertiary)", fontSize: 11 }}>
        <span>{view === "day" ? label(dayWeeks[0]?.start ?? Date.now(), "day") : view === "week" ? label(weekBuckets[0]?.start ?? Date.now(), "week") : label(monthBuckets[0]?.start ?? Date.now(), "month")}</span>
        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>用量低　{[0, 1, 2, 3, 4].map((level) => <span key={level} style={{ width: 9, height: 9, borderRadius: 2, background: cellColor(level), display: "inline-block" }} />)}　用量高</span>
