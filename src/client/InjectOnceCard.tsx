...
export function InjectOnceCard({ scope }: { scope: SettingsScope<InjectOnceSettings> }) {
  ...
  return <li style={{ listStyle: 'none', border: '1px solid var(--dsw-alias-border-l2)', borderRadius: 12, background: 'var(--dsw-alias-bg-layer-3)' }}>
    <button type="button" aria-expanded={open} aria-label={`${open ? '收起' : '展开'}：备忘录`} onClick={() => setOpen(!open)} style={{ width: '100%', appearance: 'none', border: 0, background: 'none', font: 'inherit', color: 'inherit', textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 12 }}>
      <span style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 4 }}>
        <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--dsw-alias-label-primary)' }}>备忘录</span>
        <span style={{ fontSize: 13, color: 'var(--dsw-alias-label-tertiary)' }}>在会话开始及压缩后注入提示词</span>
      </span>
    ...
    {open ? <div style={{ borderTop: '1px solid var(--dsw-alias-border-l2)', margin: '0 16px', padding: '14px 0 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <p style={{ margin: 0, fontSize: 12, lineHeight: 1.5, color: 'var(--dsw-alias-label-tertiary)' }}>项目根目录的 AGENTS.md 会自动追加在全局提醒之后</p>
